...
def erase_data(ticker, interval="daily", stage="all"):
    ...
    stage (str): Which tier to delete - 'bronze', 'silver', 'gold', 'both' (bronze+silver), or 'all'.
    """
    ...
    if stage_lower == "all":
        stages_to_delete = ["bronze", "silver", "gold"]
    elif stage_lower == "both":
        stages_to_delete = ["bronze", "silver"]
    elif stage_lower in ["bronze", "silver", "gold"]:
        stages_to_delete = [stage_lower]
    else:
        print(f"Error: Invalid stage '{stage}'. Choose 'bronze', 'silver', 'gold', 'both', or 'all'.")
        return
    ...
    for s in stages_to_delete:
        ...
        # gold TIER: Delete all varying parquet files (e.g., baseline_results.parquet)
        if s == "gold":
                ...
                for file in os.listdir(stage_dir):
                    if (file.endswith(".parquet") | (file.endswith(".json"))):
                        try:
                            ...
                            print(f"[SUCCESS] Erased {file} in gold for {ticker}.")
                            files_deleted += 1
                        except Exception as e:
                            print(f"[ERROR] Could not delete {file}: {e}")
                if files_deleted == 0:
                    print(f"[SKIP] No .parquet files found to delete in gold for {ticker}.")
            else:
                print(f"[SKIP] No gold directory found for {ticker} at: {stage_dir}")
                
        # bronze & silver TIERS: Delete the master data.parquet file
        else:
...
if __name__ == "__main__":
    ...
    stage = input("Enter the stage to erase ('bronze', 'silver', 'gold', 'both', 'all') [default: 'all']: ").strip().lower() or "all"
